...
export (Array, String, MULTILINE) var lines := ["The city is counting on us.", "Which problem should we solve first?", "Every choice changes the city."]
...
func set_civic_dialogue():
	if !is_npc: return

	var district = Shared.map_name
	if district == "":
		district = Shared.csfn.right(Shared.worlds_path.length()).replace(".tscn", "")
	district = district.split("/")[0]

	match district:
		"1":
			lines = ["The main road is blocked.", "Emergency vehicles need a clear lane.", "Please check the crossing first.", "One shortcut can affect the whole block.", "Keep the bus route moving.", "People are waiting for a safe route.", "Traffic changes when you move one thing.", "Can you open a path for everyone?"]
		"2":
			lines = ["The water pressure is dropping.", "A small leak can waste a lot.", "Which homes need water first?", "Close the line before the tank empties.", "Every drop matters today.", "The clinic needs a reliable supply.", "Protect the neighborhood reserve.", "Can you route water where it matters?"]
		"3":
			lines = ["The collection route is blocked.", "Bins cannot stay in the emergency lane.", "Sort the waste before pickup.", "A clean street helps everyone.", "Keep the truck route open.", "One missed stop affects the whole district.", "Good systems prevent overflowing bins.", "Can you make a cleaner route?"]
		"2A":
			lines = ["The warning route must stay clear.", "Families need a safe way out.", "Follow the signs to higher ground.", "Do not block the emergency exit.", "Check on the people nearby.", "A calm plan saves precious time.", "Every turn should lead to safety.", "Can you open the evacuation path?"]
		"3A":
			lines = ["The district has lost power.", "The clinic needs the next connection.", "Do not overload the circuit.", "Find a route around the damaged line.", "Keep the signals working.", "Power must reach essential services first.", "One connection can restore a block.", "Can you stabilize the grid?"]
		"2B":
			lines = ["The sidewalk ends at the busy road.", "Children need a safer crossing.", "Give people time to cross.", "Keep the cycle lane visible.", "A safe street belongs to everyone.", "Move the barrier away from the ramp.", "Watch for people before traffic.", "Can you build a green path?"]
		"2C":
			lines = ["This district is under pressure.", "Resources must reach every street.", "Look for the choice with less waste.", "The city works as one system.", "Check who is affected first.", "A small fix can protect a neighborhood.", "Leave room for the next response.", "What will your decision change?"]
		"3B":
			lines = ["Neighbors are working together.", "Share the safest route with everyone.", "No one should be left behind.", "The community needs a clear signal.", "Listen before you move the plan.", "Reliable streets build public trust.", "Help the next person succeed.", "A stronger city starts locally."]
		_:
			lines = ["The city is counting on us.", "Look around before you act.", "A safe solution helps more people.", "Check the route and the people on it.", "Every block is connected.", "Small choices create large changes.", "Help the community move forward.", "Every choice changes the city."]
